...
class PushTEnv(gym.Env):
    ...
    def _set_state_local(self, state_local):
        ...
        new_state = np.array(
            list(agent_pos_new[0]) + list(tf_img_new.translation) + [tf_img_new.rotation]
        )
    ...
    def add_tee(
        self, position, angle, scale=30, color="LightSlateGray", mask=pymunk.ShapeFilter.ALL_MASKS()
    ):
        mass = 1
        length = 4
        vertices1 = [
