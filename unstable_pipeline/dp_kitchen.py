...
def inference(args, envs, dataset, agent, logger):
    ...
    for i in range(args.eval_episodes // args.num_envs): 
        ...
        while t < args.max_episode_steps:
            ...
            nobs = nobs = torch.tensor(nobs, device=args.device, dtype=torch.float32)  # (num_envs, obs_steps, obs_dim)
            with torch.no_grad():
            ...
            naction = naction.detach().to('cpu').numpy().clip(-1, 1)  # (num_envs, horizon, action_dim)
        ...
        print(f"[Episode {1+i*(args.num_envs)}-{(i+1)*(args.num_envs)}] reward: {ep_reward} success:{success} kit_success:{kit_success}")
        episode_rewards.append(ep_reward)
        episode_steps.append(t)
        episode_success.append(success)
        episode_kit_success.append(kit_success)
    
    eval_metrics = {'mean_step': np.nanmean(episode_steps), 'mean_reward': np.nanmean(episode_rewards), 'mean_success': np.nanmean(episode_success)}
...
@hydra.main(config_path="../configs/dp/kitchen/dit", config_name="kitchen_abs")
def pipeline(args):
    # ---------------- Create Logger ----------------
    set_seed(args.seed)
    ...
    if args.abs_action:
        dataset = KitchenMjlDataset(dataset_path, horizon=args.horizon, pad_before=args.obs_steps-1, pad_after=args.action_steps-1)
    else:
        dataset = KitchenDataset(dataset_path, horizon=args.horizon, pad_before=args.obs_steps-1, pad_after=args.action_steps-1)
    ...
    if args.nn == "dit":
        ...
        nn_diffusion = ChiTransformer(
            args.action_dim, args.obs_dim, args.horizon, args.obs_steps, d_model=768, nhead=4, num_layers=8,
