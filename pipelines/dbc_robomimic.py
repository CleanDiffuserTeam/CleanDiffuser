...
@hydra.main(config_path="../configs/dbc/robomimic/pearce_mlp", config_name="lift")
def pipeline(args):
    # ---------------- Create Logger ----------------
    set_seed(args.seed)
    ...
    if args.diffusion == "ddpm":
        ...
        x_max = torch.ones((1, args.action_dim), device=args.device) * +1.0
        x_min = torch.ones((1, args.action_dim), device=args.device) * -1.0
        agent = DDPM(
    ...
    if args.mode == "train":
        ...
        metrics.update(inference(args, envs, dataset, agent, logger))
        if not os.path.exists(f"/home/ubuntu/yyf/cleandiffuser/{args.exp_name}/"):
            os.makedirs(f"/home/ubuntu/yyf/cleandiffuser/{args.exp_name}/")
        import json
        with open(f'/home/ubuntu/yyf/cleandiffuser/{args.exp_name}/metrics.jsonl', 'w') as f:
