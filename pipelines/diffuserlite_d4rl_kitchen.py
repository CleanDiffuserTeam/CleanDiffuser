...
from cleandiffuser.dataset.d4rl_kitchen_dataset import MultiHorizonD4RLKitchenDataset, D4RLKitchenTDDataset
...
from cleandiffuser.utils import set_seed, loop_dataloader, dict_apply
from cleandiffuser.utils.iql import IQL


class MultiHorizonD4RLKitchenDatasetwQ(MultiHorizonD4RLKitchenDataset):
    pred_values = None
...
@hydra.main(config_path="../configs/diffuserlite/kitchen", config_name="kitchen", version_base=None)
def pipeline(args):
    set_seed(args.seed)
    if args.test_model == "R2":
        args.diffusion_ckpt = args.reflow_ckpt
    else:
        args.diffusion_ckpt = args.task.diffusion_ckpt

    w_cfgs = [1.0, 1.0, 1.0]
    tgts = [0.3, 0.35, 0.4, 0.5] if "mixed" in args.task.env_name else [0.25, 0.35, 0.45, 0.5]

    save_path = f'results/{args.pipeline_name}/{args.task.env_name}/'
    if os.path.exists(save_path) is False:
        os.makedirs(save_path)

    planning_horizons = [4, 5, 5]
    # ========================== Level Setup ==========================
    ...
    dataset = MultiHorizonD4RLKitchenDatasetwQ(
    ...
    if args.mode == "iql_training":
        ...
        for batch in loop_dataloader(td_dataloader):
            ...
            rew = batch["rew"].to(args.device) / 4.
        ...
        for batch in loop_dataloader(dataloader):
            for i in range(n_levels):
                ...
                rew = batch[i]["data"]["rew"].to(args.device) / 4.
                pred_val = batch[i]["data"]["pred_val"].to(args.device)

                if i == 0:
                    rew[:, -1] = pred_val[:, -1]
                    val = (disc_tensor * rew).sum(1) / 100.
                else:
                    val = rew.mean(1)
        ...
        for batch in loop_dataloader(dataloader):
            for i in range(n_levels):
                ...
                if i == 0:
                    rew[:, -1] = pred_val[:, -1]
                    val = (args.discount ** torch.arange(rew.shape[1], device=args.device)[None, :, None].float() *
                           rew).sum(1) / 100.
                else:
                    val = rew.mean(1)
        ...
        for i in range(args.num_episodes):
            ...
            while not np.all(cum_done) and t < 280 + 1:
                ...
                for k in range(4):
                    target_return[ep_reward == k] = tgts[k]
                ...
                print(f'[t={t}] rew: {ep_reward} | '
                      f'[{(ep_reward >= 1).sum() / 50}, {(ep_reward >= 2).sum() / 50}, {(ep_reward >= 3).sum() / 50}]')
