...
@hydra.main(config_path="../configs/adaptdiffuser", config_name="d4rl", version_base=None)
def pipeline(args):
    ...
    # --- Fintuning ---
        ...
        actor.load_state_dict(
            torch.load(save_path / f"diffusion-step={args.diffusion_ckpt}.ckpt", map_location=device)["state_dict"]
        )
        actor.eval().to(device)

        traj_buffer = torch.empty((traj_buffer_sz, args.task.horizon, obs_dim + act_dim), device=device)
        sample_bs, ptr = 20000, 0

        gen_dl = DataLoader(dataset, batch_size=sample_bs, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
        prior = torch.zeros((sample_bs, args.task.horizon, obs_dim + act_dim))
        for batch in loop_dataloader(gen_dl):
            prior[:, 0, :obs_dim] = batch["obs"]["state"][:, 0]
            ...
            if ptr + num_selected > traj_buffer_sz:
                num_selected = traj_buffer_sz - ptr
                selected_traj = selected_traj[:num_selected]
            traj_buffer[ptr : ptr + num_selected] = selected_traj
            ptr += num_selected

            print(f"{num_selected} of {sample_bs} trajs have been selected. Progress: {ptr} / {traj_buffer_sz}")
            if ptr == traj_buffer_sz:
                break

        # self-evolving finetuning
        actor.train()
        actor.optimizer_params["diffusion"]["lr"] = 1e-5

        n_gradient_step = 0
        log = {"avg_loss_diffusion": 0.0, "gradient_steps": 0}
        while n_gradient_step < 1000:
            x = traj_buffer[torch.randint(0, 50000, (512,))]
            log["avg_loss_diffusion"] += actor.update_diffusion(x)["diffusion_loss"]
            if (n_gradient_step + 1) % 100 == 0:
                log["gradient_steps"] = n_gradient_step + 1
                log["avg_loss_diffusion"] /= 100
                print(log)
                log = {"avg_loss_diffusion": 0.0, "gradient_steps": 0}
            if (n_gradient_step + 1) % 1000 == 0:
                torch.save(
                    {"state_dict": actor.state_dict()},
                    save_path / f"finetuned_diffusion_ckpt-step={n_gradient_step + 1}.ckpt",
                )
            n_gradient_step += 1
    ...
    # --- Inference ---
        ...
        actor.load_state_dict(
            torch.load(
                save_path / f"finetuned_diffusion_ckpt-step={args.ckpt}.ckpt", map_location=f"cuda:{args.device_id}"
            )["state_dict"]
        )
