...
class SynthERD4RLAntmazeTDDataset(D4RLAntmazeTDDataset):
    def __init__(self, save_path, dataset):
        super().__init__(dataset, "iql")
        ...
        rewards = np.concatenate([rewards - 1, extra_rewards[:, None]], 0)
...
@hydra.main(config_path="../configs/synther/antmaze", config_name="antmaze", version_base=None)
def pipeline(args):
    set_seed(args.seed)
    ...
    # --------------------- Train RL ------------------------
        ...
        dataset = SynthERD4RLAntmazeTDDataset(save_path, d4rl.qlearning_dataset(env))
        dataloader = DataLoader(
            dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
