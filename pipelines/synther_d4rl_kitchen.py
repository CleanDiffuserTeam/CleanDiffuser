...
class SynthERD4RLKitchenTDDataset(D4RLKitchenTDDataset):
...
@hydra.main(config_path="../configs/synther/kitchen", config_name="kitchen", version_base=None)
def pipeline(args):
    set_seed(args.seed)
    ...
    # --------------------- Train RL ------------------------
        ...
        dataset = SynthERD4RLKitchenTDDataset(save_path, d4rl.qlearning_dataset(env))
        dataloader = DataLoader(
            dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
