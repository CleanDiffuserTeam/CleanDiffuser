...
def pipeline(args):
    ...
    # --- Inference ---
        ...
        if args.invdyn_from_pretrain:
            invdyn = FancyMlpInvDynamic.from_pretrained(env_name, hidden_dim=args.invdyn_hidden_dim)
        else:
            invdyn = FancyMlpInvDynamic.load_from_checkpoint(
                checkpoint_path=save_path / f"invdyn-step={args.invdyn_ckpt}.ckpt",
                obs_dim=obs_dim,
                act_dim=act_dim,
                hidden_dim=args.invdyn_hidden_dim,
            )
        invdyn.to(f"cuda:{args.device_id}").eval()

        actor = ContinuousDiffusionSDE.load_from_checkpoint(
            checkpoint_path=save_path / f"diffusion-step={args.diffusion_ckpt}.ckpt",
        ...
        condition = torch.ones((num_envs, 1), device=f"cuda:{args.device_id}") * args.task.target_return
